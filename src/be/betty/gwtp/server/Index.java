...
// TODO: met-t-on des assert � toutes les methodes ?
public class Index {
	...
	public void putRightIndex(String[] line, int choice_sem) {
		...
		for (int i = 0; i < line.length; i++) {
			...
			if (line[i].equalsIgnoreCase("année") || line[i].equalsIgnoreCase("anne"))
				indexLine.put(year, i);

			else if (line[i].equalsIgnoreCase("Intitulé cours") || line[i].equalsIgnoreCase("Intitule cours"))
				indexLine.put(course_name, i);

			else if (line[i].equalsIgnoreCase("Prénom") || line[i].equalsIgnoreCase("Prenom"))
				...
				// System.out.println("on a fix� l'index des periodes �: "+i);
			}
			...
			else if (line[i].equalsIgnoreCase("Intitulé Section") || line[i].equalsIgnoreCase("Intitule Section"))
